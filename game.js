...
document.addEventListener('keydown',function(e){
    if(e.key=='q' && Number(sc2.innerText)>0 && gameState==true){
        let s=pl1.strike();
        updateGame(sc1,sc2,0,s,0,0);
        p1im.src='assests/strik1.gif';
        ad1.play();
    }
    else if(e.key=='p' && Number(sc1.innerText)>0 && gameState==true){
        let s=pl2.strike();
        updateGame(sc1,sc2,s,0,0,0);
        p2im.src='assests/strik2.gif';
        ad2.play();
    }
    else if(e.key=='a' && Number(sc1.innerText)>0 && gameState==true){
        let h=pl1.heal();
        updateGame(sc1,sc2,0,0,h,0);
        ad3.play();
    }
    else if(e.key=='l' && Number(sc2.innerText)>0 && gameState==true){
        let h=pl2.heal();
